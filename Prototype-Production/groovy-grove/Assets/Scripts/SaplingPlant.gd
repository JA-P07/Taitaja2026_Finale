...
func _start_seed_step(player):
	var game = preload("res://Assets/Scenes/counter_minigame.tscn").instantiate()
	get_tree().root.add_child(game)
	player.current_minigame = game
	game.connect("completed", Callable(self, "_on_minigame_completed").bind("seedbag"))
	game.start(player)

func _on_minigame_completed(_player, step: String) -> void:
	if step == "spade":
		spade_step_done = true
		print("Spade step finished. Now use seedbag.")
	elif step == "seedbag":
		finish_planting()
